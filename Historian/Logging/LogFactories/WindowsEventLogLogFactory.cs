...
namespace Historian.Logging.LogFactories
{
    /// <summary>
    ...
    public class WindowsEventLogLogFactory : ILogFactory
    {
        ...
        private class WindowsEventLogLogger : ILog
        {
            ...
            {
                if (appIdentifier == null) throw new ArgumentNullException(nameof(appIdentifier));
                ...
                {
                    // May occur if the Security event log is locked/unreadable due to access privileges.
                    throw new Exception("Could not configure windows eventlog log factory due to insufficient rights (cannot read the Security event log). See http://stackoverflow.com/questions/1274018/system-security-securityexception-when-writing-to-event-log for more details.", ex);
                }
            }
